...
import { AnalyticsService, KPIStats, DailyStats, AnalyticsResponse } from '../../core/services/analytics.service';
          ...
          @for (rt of analytics()?.room_type_breakdown || []; track rt.room_type) {
            <div class="room-type-bar">
              ...
              <div class="room-type-bar__track">
                <div
                  class="room-type-bar__fill"
                  [style]="'width: ' + getBarWidth(rt.revenue) + '%'"
                ></div>
              </div>
  ...
  styles: [`
    ...
    .chart-card {
      ...
      &__header {
        ...
        h3 { font-size: 16px; font-weight: 700; color: var(--ib-text); }
        p  { font-size: 12px; color: var(--ib-text-muted); margin-top: 3px; }
      }

      &__total {
        text-align: right;
        span { display: block; font-size: 11px; color: var(--ib-text-muted); }
        strong { font-size: 1.2rem; font-weight: 800; color: var(--ib-primary-light); }
      }
    ...
    .donut-center {
      ...
      strong { display: block; font-size: 1.6rem; font-weight: 800; color: var(--ib-text); }
    ...
    .room-type-bar {
      ...
      &__info {
        ...
        span { font-size: 13px; color: var(--ib-text-muted); }
        strong { font-size: 13px; color: var(--ib-text); font-weight: 700; }
      }
...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private buildKPICards(kpis: any) {
    this.kpiCards.set([
      { icon: '📅', label: 'Total Bookings',   value: kpis.total_bookings.toLocaleString(), change: '+12.5%', positive: true, color: '#6366f1' },
      { icon: '💰', label: 'Total Revenue',    value: '$' + (kpis.total_revenue / 1000).toFixed(1) + 'k', change: '+29.4%', positive: true, color: '#22c55e' },
      { icon: '✅', label: 'Success Rate',     value: kpis.success_rate + '%', change: '+2.1%', positive: true, color: '#22d3ee' },
      { icon: '📊', label: 'Avg Booking Value', value: '$' + kpis.avg_booking_value.toFixed(0), change: '-3.2%', positive: false, color: '#f59e0b' },
      { icon: '🌟', label: 'Bookings Today',   value: String(kpis.bookings_today), change: '+7', positive: true, color: '#a78bfa' },
      { icon: '💵', label: 'Revenue Today',    value: '$' + kpis.revenue_today.toLocaleString(), change: '+$280', positive: true, color: '#34d399' },
      { icon: '⏳', label: 'Pending Bookings', value: String(kpis.pending_bookings), change: '-2', positive: true, color: '#fbbf24' },
      { icon: '❌', label: 'Failed Payments',  value: String(kpis.failed_payments), change: '+1', positive: false, color: '#f87171' },
    ]);
  }
  ...
  private renderCharts(data: AnalyticsResponse) {
    ...
    if (this.statusChartRef?.nativeElement) {
      ...
      const chart = new Chart(ctx, {
        type: 'doughnut',
        data: {
          labels: data.payment_breakdown.map(p => p.status),
          datasets: [{
            data: data.payment_breakdown.map(p => p.count),
  ...
  getBarWidth(revenue: number): number {
    const max = Math.max(...(this.analytics()?.room_type_breakdown || []).map(r => r.revenue));
